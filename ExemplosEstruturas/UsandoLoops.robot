...
Usando FOR IN RANGE
    Log To Console  ${\n}
    FOR  ${CONTADOR}   IN RANGE  0   5  # obs>>>  robot não conta o ultimo numero da repetição , vai de 0 até 4
...
Usando FOR IN
    Log To Console    ${\n}
    FOR  ${FRUTA}   IN  @{MINHA_LISTA_DE_FRUTAS}  # variavel FRUTA recebe valor de cada item da MINHA LISTA DE FRUTAS 
...
Usando FOR IN ENUMERATE
    Log To Console    ${\n}
    FOR   ${INDICE}   ${FRUTA}   IN ENUMERATE   @{MINHA_LISTA_DE_FRUTAS}  #Enumerate vai contar quantas vezes foi feita a posição
...
Usando FOR IN com EXIT FOR LOOP IF
    ...
    FOR   ${INDICE}   ${FRUTA}   IN ENUMERATE   @{MINHA_LISTA_DE_FRUTAS}
        Log To Console    Minha fruta é: ${INDICE} --> ${FRUTA}!
        Exit For Loop If    '${FRUTA}'=='banana'   #Ele sai do looping quando a condição criada for verdadeira
    END
